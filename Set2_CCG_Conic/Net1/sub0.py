import networkx as nx;
import gurobipy as gp;
from gurobipy import GRB;
import sys;
import time;
from datetime import datetime;
import csv;
import pandas as pd;
import math;
import random;
import numpy;
from numpy import *



def sub0 (G, s, t, budget, gamma, cMin, cMax, k, timeLimit):
    
   
    
    print('\n--- Solving Sub 0 ---\n')
       
    
    sub = gp.Model('sub0')
    
    obj = sub.addVar(vtype=GRB.INTEGER, lb = -GRB.INFINITY, ub = GRB.INFINITY, name = 'obj');
    
    c = sub.addVars(G.edges, vtype=GRB.INTEGER, lb = -GRB.INFINITY, ub = GRB.INFINITY, name = 'op_cost');
    
    y = sub.addVars(G.nodes, vtype=GRB.CONTINUOUS, lb = -GRB.INFINITY, ub = GRB.INFINITY, name='y');
    w = sub.addVars(G.edges, vtype=GRB.CONTINUOUS, lb = -GRB.INFINITY, ub = 0, name='w');
    p = sub.addVars(G.edges, vtype=GRB.CONTINUOUS, lb = -GRB.INFINITY, ub = 0, name='p');

    
    # Setting operational costs:   
    random.seed(1);
    
    for i,j in G.edges:
        if j == t:
            if i == s:
                sub.addConstr(c[i,j] == 0);
            else:
                center = random.randint(cMin, cMax);
                # print('(%d,%d) range = [%d,%d]' %(i,j,center-2,center+2));
                sub.addConstr(c[i,j] >= center-2);
                sub.addConstr(c[i,j] <= center+2);
        else:
            sub.addConstr(c[i,j] == 0);
    ###############################################################

    # Setting operational costs:      
    
    # for i,j in G.edges:
    #     if j == t:
    #         if i == s:
    #             sub.addConstr(c[i,j] == 0);
    #         else:
    #             sub.addConstr(c[i,j] >= cMin);
    #             sub.addConstr(c[i,j] <= cMax);
    #     else:
    #         sub.addConstr(c[i,j] == 0);
    ###############################################################
            

    for i,j in G.edges:
        sub.addConstr(p[i,j] >= 0);
        
        if G.edges[i,j]['special'] == 1:
            sub.addConstr(y[i] - y[j] + w[i,j] <= 1);
            
        else:
            sub.addConstr(y[i] - y[j] + w[i,j] <= 0);
            
    sub.addConstrs(gp.quicksum(p[i,j] for j in G.successors(i))- gp.quicksum(p[j,i] for j in G.predecessors(i)) == 0 for i in G.nodes);

    # Integer Objective Value
    sub.addConstr(gp.quicksum(G.nodes[i]['demand']*y[i] for i in G.nodes) 
                                + gp.quicksum(G.edges[i,j]['capacity']*(1 - gamma[i,j])*w[i,j] for i,j in G.edges)
                                + gp.quicksum(c[i,j]*p[i,j] for i,j in G.edges) == obj);
    

    sub.setObjective(gp.quicksum(G.nodes[i]['demand']*y[i] for i in G.nodes) 
                                + gp.quicksum(G.edges[i,j]['capacity']*(1 - gamma[i,j])*w[i,j] for i,j in G.edges)
                                + gp.quicksum(c[i,j]*p[i,j] for i,j in G.edges), GRB.MAXIMIZE);
    
    
    
    
    sub.setParam("NonConvex", 2);
    
    sub.setParam("OutputFlag", 0);
    sub.setParam("LogToConsole", 0);
    sub.setParam("MIPFocus", 3);
    sub.setParam("Cuts", 3);
    sub.setParam("TimeLimit", timeLimit);
    
    sub.update();
    sub.optimize();
    
        
    UB = 10e8;
    gap = 10e8;
    LB = 0;
    
    C = {};
    for i,j in G.edges:
        C[i,j] = 0;
    
    if sub.status == 3:
        print('\nInfeasible!');

    elif sub.status == 4:
        print('\nInfeasible or Unbounded!');

    elif sub.status == 2:
        print('\nSolved to optimality!')
        
        for i,j in G.edges:
            C[i,j] = c[i,j].x;
            
        UB = sub.ObjBound;
        LB = sub.ObjVal;
        gap = sub.MIPGap;
        
    elif sub.status == 9:
        print('\nReached Time limt!')
        
        if sub.SolCount == 0:
            print('No feasible solution found!\n')
            return None;

        else:
            print('MIP Gap %g\n' %sub.MIPGap);  
        
            for i,j in G.edges:
                C[i,j] = c[i,j].x;
            
            UB = sub.ObjBound;
            LB = sub.ObjVal;
            gap = sub.MIPGap;
    else:
        print('\nModel status: %d' %sub.status);
      
    
    return C, LB, UB, gap;